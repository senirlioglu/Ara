...
def _get_oneri_listesi_impl():
    """Autocomplete için ürün isimlerini veritabanından getir"""
    debug_info = []
    try:
        client = get_supabase_client()
        if not client:
        ...
        # 1. RPC ile benzersiz ürün adlarını getir (Hızlı Yöntem)
        # Limit 20,000 yapıldı (15,100 benzersiz ürün olduğu belirtildi)
        try:
            result = client.rpc('get_urun_adlari', {'result_limit': 20000}).execute()
            if result.data:
                # Gelen veriyi işle
                liste = []
                seen = set()
                ...
                if liste:
                    ...
                    # Alfabetik sırala ama limitleme (tümünü gönder)
        ...
        # 2. Fallback: Tablodan direkt çek (Eğer RPC yoksa)
        # Sadece ilk 5000 benzersiz kaydı al (performans için)
        try:
            # Not: Supabase'de 'distinct' seçeneği olmadığı için RPC daha iyidir.
            # Ancak mecbur kalırsak normal select deneriz.
...
def admin_panel():
    ...
    if not admin_pass:
        st.error("Admin şifresi ayarlanmamış!")
        return
